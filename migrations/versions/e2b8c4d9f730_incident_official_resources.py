...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("kind IN ('safety', 'press', 'official_update', 'authority')", name="ck_official_resource_kind"),
        sa.CheckConstraint("state IN ('PROPOSED', 'PUBLISHED', 'REJECTED', 'RETIRED')", name="ck_official_resource_state"),
        sa.CheckConstraint("url LIKE 'https://%'", name="ck_official_resource_https"),
        sa.CheckConstraint("source_reference_url LIKE 'https://%'", name="ck_official_resource_source_https"),
        sa.CheckConstraint("version >= 1", name="ck_official_resource_version"),
    )
    op.create_index("ix_incident_official_resource_resource_id", "incident_official_resource", ["resource_id"], unique=True)
    op.create_index("ix_incident_official_resource_incident_id", "incident_official_resource", ["incident_id"])
    op.create_index("ix_incident_official_resource_episode_id", "incident_official_resource", ["episode_id"])
    ...
    op.create_index("ix_official_resource_incident_state", "incident_official_resource", ["incident_id", "state"])
...
def downgrade() -> None:
    ...
    op.drop_index("ix_incident_official_resource_episode_id", table_name="incident_official_resource")
    op.drop_index("ix_incident_official_resource_incident_id", table_name="incident_official_resource")
    op.drop_index("ix_incident_official_resource_resource_id", table_name="incident_official_resource")
